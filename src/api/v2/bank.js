...
async function getTableData({ app, objectId, width = 20, height = 100 }) {
     const object = await app.getObject(objectId);
...
module.exports = {
     get: async (req, res) => {
          try {
               ...
               const app = await session.openDoc(
                    BANK_CONFIG.appId,
                    "",
                    "",
                    "",
               ...
               const table1Object = await app.getObject(
                    BANK_CONFIG.tavsiye_cizelge_table
               );
               const cubeData1 = await table1Object.getHyperCubeData(
                    "/qHyperCubeDef",
                    [
                         {
                              qLeft: 0,
               ...
               const [{ qMatrix: faizOranlariVeMevduatlarData = [] } = {}] =
                    cubeData1 || [];

               const { data: subeData, subTotals: subeSubTotals } =
                    await getTableData({
                         app,
               ...
               const {
                    ...
                    app,
                    objectId: BANK_CONFIG.kpi_baglanan_para,
                    useColoring: false,
               });

               const {
                    value: kpi_bankadaki_para,
                    style: kpi_bankadaki_para_style,
               } = await getKpiData({
                    app,
                    objectId: BANK_CONFIG.kpi_bankadaki_para,
                    useColoring: false,
               });

               const {
                    value: kpi_blokeli_para,
                    style: kpi_blokeli_para_style,
               } = await getKpiData({
                    app,
                    objectId: BANK_CONFIG.kpi_blokeli_para,
                    useColoring: false,
               });

               const { value: kpi_toplam_para, style: kpi_toplam_para_style } =
                    await getKpiData({
                         app,
                         objectId: BANK_CONFIG.kpi_toplam_para,
                         useColoring: false,
                    });

               const { value: kpi_gecmis_fkv, style: kpi_gecmis_fkv_style } =
                    await getKpiData({
                         app,
                         objectId: BANK_CONFIG.kpi_gecmis_fkv,
                         useColoring: false,
                    });

               const { value: kpi_kalan_fkv, style: kpi_kalan_fkv_style } =
                    await getKpiData({
                         app,
                         objectId: BANK_CONFIG.kpi_kalan_fkv,
                         useColoring: false,
                    });

               const { value: kpi_toplam_fkv, style: kpi_toplam_fkv_style } =
                    await getKpiData({
                         app,
                         objectId: BANK_CONFIG.kpi_toplam_fkv,
                         useColoring: false,
                    });

               const { value: kpi_gecmis_fg, style: kpi_gecmis_fg_style } =
                    await getKpiData({
                         app,
                         objectId: BANK_CONFIG.kpi_gecmis_fg,
                         useColoring: true,
                    });

               const { value: kpi_kalan_fg, style: kpi_kalan_fg_style } =
                    await getKpiData({
                         app,
                         objectId: BANK_CONFIG.kpi_kalan_fg,
                         useColoring: true,
                    });

               const { value: kpi_toplam_fg, style: kpi_toplam_fg_style } =
                    await getKpiData({
                         app,
                         objectId: BANK_CONFIG.kpi_toplam_fg,
                         useColoring: true,
                    });

               const { value: kpi_gecmis_fark, style: kpi_gecmis_fark_style } =
                    await getKpiData({
                         app,
                         objectId: BANK_CONFIG.kpi_gecmis_fark,
                         useColoring: true,
                    });

               const { value: kpi_kalan_fark, style: kpi_kalan_fark_style } =
                    await getKpiData({
                         app,
                         objectId: BANK_CONFIG.kpi_kalan_fark,
                         useColoring: true,
                    });

               const { value: kpi_toplam_fark, style: kpi_toplam_fark_style } =
                    await getKpiData({
                         app,
                         objectId: BANK_CONFIG.kpi_toplam_fark,
                         useColoring: true,
                    });

               const {
                    value: kpi_odenecek_anapara,
                    style: kpi_odenecek_anapara_style,
               } = await getKpiData({
                    app,
                    objectId: BANK_CONFIG.kpi_odenecek_anapara,
                    useColoring: true,
               });
